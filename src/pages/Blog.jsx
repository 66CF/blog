...
export default function Blog() {
  return (
    <div className="flex-1 overflow-y-auto scrollbar-none">
      ...
      <p className="text-gray-400 text-sm mt-2 mb-8">writing about things I learn and build</p>
      ...
      {blogPosts && blogPosts.length > 0 ? (
        <div className="grid gap-6">
          {blogPosts.map((post) => (
            ...
            >
              <Link to={`/blog/${post.slug}`} className="block group-hover:text-accent">
                <div className="flex items-baseline justify-between gap-4">
                  <p className="text-xl font-bold">{post.title}</p>
                  <span className="text-gray-500 text-xs font-mono whitespace-nowrap">{post.date}</span>
                </div>
